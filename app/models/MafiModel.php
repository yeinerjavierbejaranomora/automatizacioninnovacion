...
class MafiModel{
    ...
    function dataMafi($offset){
        try {
            $consulta = $this->db->connect()->prepare("SELECT * FROM `datosMafi` WHERE `id` > 2 AND `estado` = 'Activo' AND `sello` IN ('TIENE RETENCION', 'TIENE SELLO FINANCIERO') ORDER BY `id` ASC");
